    ...
    for i in range(5):
        ...
        if rank == 0:
            for non_parallel_model_parameter, ddp_model_parameter in zip(
                ...
                assert torch.allclose(
                    non_parallel_model_parameter, ddp_model_parameter
                ), f"Parameters do not match after iteration {i}. Non-parallel param: {non_parallel_model_parameter}, DDP param: {ddp_model_parameter}. Relative difference: {torch.abs(non_parallel_model_parameter - ddp_model_parameter) / torch.abs(non_parallel_model_parameter)}"
